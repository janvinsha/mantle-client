...
import { useNftOfAddress } from '../hooks/useNftOfAddress';
...
export default function Profile() {
  ...
  let userDetails = {};
  const { collectionList } = useCollectionOfAddress(handle);

  const collections = collectionList;
  const { profile } = useProfile(handle);
  let foundUser = profile;

  const { nftList } = useNftOfAddress(handle);
  const nfts = nftList;
  return (
    <StyledProfile
      exit="exit"
